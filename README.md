...
合併去重後最多顯示 100 個頁面，使用者可勾選最多 10 頁進行深度分析。
...
每個頁面分析完成後，自動呼叫 OpenAI GPT-4o-mini，根據七大指標的分析資料產生一份繁體中文 SEO 健診報告，包含：
- 整體評估（1-2 句概述）
- 主要問題（依嚴重程度排序，最多 5 點）
- 優先改善建議（具體可執行，最多 3 點）
...
1. 以 Google 帳號登入
        ↓
2. 在 Dashboard 輸入網域（例：example.com）
        ↓
3. 系統掃描 sitemap.xml + 首頁，顯示最多 100 頁的清單
        ↓
4. 勾選要分析的頁面（最多 10 頁）→ 開始分析
        ↓
5. 進度頁面實時顯示分析進度（每 3 秒更新）
        ↓
6. 分析完成 → 查看完整報告
   ├── 左側：頁面清單（✅ ⚠️ ❌ 標示問題數量）
   └── 右側：7 大指標詳情 + AI 中文健診報告
        ↓
7. 歷史紀錄頁面可回顧所有過去的分析
```
